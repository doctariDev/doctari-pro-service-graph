...
program
    ...
    .description("Analyze service and repository dependencies. Produces a dot and a png file.")
...
function analyze(serviceDirectory) {
    ...
    const createDotFileContent = function (serviceMap, repositoryMap) {
        function writeFillcolor(content, serviceName, color) {
            return content + `  ${serviceName} [ fillcolor="${color}" style=filled ]
`;
        }
        function hasOnlyOneRepoAsDependency(dependencies) {
            return dependencies.length === 1 && dependencies[0].indexOf('Repository') > -1;
        }
        function isDependentRepoUsedOnlyByThisService(repositoryMap, serviceName, dependencies) {
            const repo = dependencies[0];
            return repositoryMap[repo].length === 1 && repositoryMap[repo][0] === serviceName;
        }
        ...
        Object.keys(serviceMap).forEach(serviceName => {
            ...
            // Already migrated
            if (dependencies.filter(dependency => dependency === 'wrapMigratedService').length > 0) {
                content = writeFillcolor(content, serviceName, 'green');
                dependencies.filter(dependency => !isIgnoredElement(dependency))
                    .forEach(dependency => {
            ...
            } else if (hasOnlyOneRepoAsDependency(dependencies) && isDependentRepoUsedOnlyByThisService(repositoryMap, serviceName, dependencies)) {
                // Only one repository
...
`;
    ...
    for (const [key, value] of Object.entries(serviceMap)) {
        value.filter(e => e.indexOf('Repository') > -1).forEach(element => {
...
`;
    ...
    exec("docker ps", (error, stdout, stderr) => {
        ...
        exec("cat services.dot | docker run --rm -i nshine/dot > services.png", (error, stdout, stderr) => {
            if (error) {
